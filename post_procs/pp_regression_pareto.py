...
for ds_name, df in model_summary_dfs.items():
    if ds_name in [
    "model_summary_Abalone",
    "model_summary_BNG_lowbwt",
    "model_summary_ERA",
    "model_summary_ESL",
    "model_summary_Wine_Quality",
    "model_summary_analcatdata_apnea1",
    "model_summary_cpu_small",
    "model_summary_houses",
    "model_summary_lev",
    "model_summary_pm10",
    "model_summary_pol",
    "model_summary_pollen",
    "model_summary_puma8NH",
    "model_summary_satellite_image",
    "model_summary_wind"
    ] :    
        if df is None or df.shape[0] == 0:
            continue
...
summary_df = summary_df[~summary_df['model'].isin(
    ['RBF','CIEL','PCE','TabPFN','IDW','KPLS','RMTS']
)]
...
# Group 1: orange
group1 = [
    'BayesianRidge','ElasticNet','Huber','LeastSquares',
    'LinearSVR','RidgeRegressor','SGDRegressor','Poisson',
    'Tweedie','MLP','QP','SGP','SVR', 'PassiveAggressive'
]

# Group 2/3: green
group2_3 = [
    'AdaBoost','GradientBoosting','HistGradientBoosting','CatBoost',
    'RandomForest','LightGBM','DecisionTree','XGBoost','ExtraTrees',
    'KNN','CIEL','IDW','TabPFN','KPLS'
]
...
valid_models = [m for m, v in acc_vals.items() if len(v) == 15]
# filter plot_df
...
ax.set_xlabel('RMSE')  # lower is better for RMSE
...
ax.set_xlabel('Mean RMSE', fontsize=16)
#ax.set_ylabel('Mean total time (training_time + shap_time) [seconds]')
...
plt.tight_layout()
plt.savefig("out_pareto_all.png", dpi=150)
plt.show()
