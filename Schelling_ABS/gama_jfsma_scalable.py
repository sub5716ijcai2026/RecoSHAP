...
def main(n_cols, ppl_dens, soc_tol, map_size, perception_dist):
    ...
    clients[0].sync_connect(True)
    for i in range(0, nb_clients):

        parameters[-1]["value"] = str(i + 1)
        gama_response = clients[0].sync_load(
    ...
    while compt < 20 and not (np.min(happy)):
        ...
        for i in range(0, nb_clients):
            if not (happy[i]):
                gama_response = clients[0].sync_expression(
                ...
                if float(gama_response["content"]) > 1 - 1e-10:
                    happy[i] = True
                    clients[0].sync_close_connection()
                else:
                    groups.append(
                        asyncio.gather(
                            async_step_simulation(clients[0], exp_id[i], loop),
                        )
                    )
